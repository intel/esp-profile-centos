...
Intended to be used with [Retail Node Installer](https://github.com/intel/retail-node-installer) and Centos base profile, this Centos OS profile contains a few files that ultimately will install Centos OS to disk.

## Software Stack in this profile

* Centos Linux w/ Docker
...
Out of the box, the Centos profile should _just work_. Therefore, no specific steps are required in order to use this profile that have not already been described in the Retail Node Installer documentation. Simply boot a client device using legacy BIOS PXE boot and the Centos profile should automatically launch after a brief waiting period.
...
After installing Centos, the default login username is `sys-admin` and the default password is `P@ssw0rd!`. This password is defined in the `bootstrap.sh` script and in the `conf/config.yml` as a kernel argument.
...
profiles:
  - git_remote_url: https://github.com/intel/rni-profile-base-Centos.git
    profile_branch: slim
    profile_base_branch: master
    git_username: ""
    git_token: ""
    name: Centos_with_Docker
